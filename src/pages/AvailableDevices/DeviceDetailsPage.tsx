  ...
  useEffect(() => {
    const loadDevice = async () => {
      ...
      if (!window.devices?.get) {
        setError("Device details are only available in the Electron application.");
        setIsLoading(false);
        return;
      }
        ...
        <section className="flex flex-1 flex-col">
          <div className="flex flex-col gap-6 rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-8 shadow-xl">
            <header className="flex flex-col gap-2 text-center md:text-left">
              <p className="text-sm uppercase tracking-widest text-emerald-400">Device Details</p>
            ...
            {!isLoading && !error && device ? (
              <dl className="grid gap-5 rounded-2xl border border-slate-800 bg-slate-900/60 p-6 md:grid-cols-2">
                <div>
                  ...
                  <dd className="text-base font-medium text-white">{device.ip}</dd>
                </div>
                <div>
                  <dt className="text-xs uppercase tracking-wide text-slate-400">Hostname</dt>
                  <dd className="text-base font-medium text-white">{device.hostname ?? "—"}</dd>
                </div>
                <div>
                  <dt className="text-xs uppercase tracking-wide text-slate-400">Alias</dt>
                  <dd className="text-base font-medium text-white">{device.alias ?? "—"}</dd>
                </div>
                <div>
                  <dt className="text-xs uppercase tracking-wide text-slate-400">Port</dt>
                  <dd className="text-base font-medium text-white">{device.port ?? 22}</dd>
                </div>
                <div>
                  <dt className="text-xs uppercase tracking-wide text-slate-400">Username</dt>
                  <dd className="text-base font-medium text-white">{device.username ?? "—"}</dd>
                </div>
                <div>
                ...
                <div>
                  ...
                  <dd className="text-base font-medium text-white">{formatTimestamp(device.createdAt)}</dd>
                </div>
                <div>
                  <dt className="text-xs uppercase tracking-wide text-slate-400">Updated</dt>
                  <dd className="text-base font-medium text-white">{formatTimestamp(device.updatedAt)}</dd>
                </div>
              </dl>
            ) : null}
